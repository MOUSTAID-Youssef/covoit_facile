...
import {
  ...
  FaSync, FaExclamationTriangle, FaCheckCircle, FaClock,
  FaChartBar, FaArrowUp, FaEye, FaWifi, FaServer
} from 'react-icons/fa';
import adminService from '../../services/adminService';

const AdminStatsWithFallback = () => {
  ...
  const defaultStats = {
    ...
    // Nouveaux états de vérification (basés sur photo_profil OU cin_path)
    users_verifie: 12,        // Utilisateurs vérifiés (document accepté par admin)
    users_en_attente: 8,      // Utilisateurs en attente (document uploadé, pas encore traité)
    users_non_verifie: 5,     // Utilisateurs non vérifiés (pas de document)
    pending_verifications: 8, // Total en attente (pour compatibilité)
    active_users: 22,
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      setStats(defaultStats);
      setUseDefaultData(true);
      setError('Serveur non disponible - Données de démonstration');
    } finally {
      setLoading(false);
    }
  };
        ...
        {useDefaultData && (
          <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="font-medium text-yellow-900">Mode Démonstration</h3>
                <p className="text-sm text-yellow-700">
                  {error} - Les données affichées sont des exemples pour la démonstration.
                </p>
                <button
                  onClick={() => navigate('/admin-api-diagnostic')}
...
export default AdminStatsWithFallback;
